// extract_advanced_stats.js
// Extracts possessions_per_game, assist_rate, and offensive_rebound_pct
// from the "Advanced School Stats" table.
//
// HOW TO USE:
// 1. Open https://www.sports-reference.com/cbb/seasons/men/2026-school-stats.html
// 2. Click the "Advanced School Stats" tab
// 3. Open DevTools console (F12)
// 4. Paste this entire script and press Enter
// 5. JSON will be copied to your clipboard (or a text box appears if clipboard is blocked)
// 6. Save the output as: import/stats_advanced.json
...
const allByName = {};
for (const { name, ...stats } of Object.values(allSchools)) {
  allByName[name] = stats;
}
...
for (const ourSlug of OUR_TEAMS) {
  ...
  if (allSchools[ourSlug]) {
    const { name, ...s } = allSchools[ourSlug];
    stats = s;
  } else if (FALLBACK_NAMES[ourSlug] && allByName[FALLBACK_NAMES[ourSlug]]) {
    stats = allByName[FALLBACK_NAMES[ourSlug]];
  }
  const dataId = ourSlug.replace(/_/g, '-');
  if (stats) out[dataId] = stats;
...
const summary = `Done! ${matched} of ${OUR_TEAMS.length} teams matched.`
  + (unmatched.length ? `\n\nNo match found for:\n  ${unmatched.join(', ')}` : '')
  + `\n\nSave output as: import/stats_advanced.json`;
