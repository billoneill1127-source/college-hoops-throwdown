// extract_adv_opp_stats.js
// Extracts defensive_rebound_pct from the "Advanced Opponent Stats" table.
// defensive_rebound_pct = (100 - opponent ORB%) / 100
//
// HOW TO USE:
// 1. Open https://www.sports-reference.com/cbb/seasons/men/2026-school-stats.html
// 2. Click the "Advanced Opponent Stats" tab
// 3. Open DevTools console (F12)
// 4. Paste this entire script and press Enter
// 5. JSON will be copied to your clipboard (or a text box appears if clipboard is blocked)
// 6. Save the output as: import/stats_adv_opp.json
...
for (const row of table.querySelectorAll('tbody tr')) {
  ...
  const maxIdx = Math.max(schoolIdx, orbPctIdx);
  ...
  // ORB% here is opponent offensive rebound %; our defensive rebound % = 100 - this
  const orbPct = parseFloat(cells[orbPctIdx].textContent.trim());
  if (isNaN(orbPct)) continue;

  const drbPct = Math.round((100 - orbPct) * 100) / 10000;  // convert to 0–1
  allSchools[toSlug(schoolName)] = { name: schoolName, defensive_rebound_pct: drbPct };
}

// ── Reverse lookup by display name ───────────────────────────────────────────
const allByName = {};
for (const { name, defensive_rebound_pct } of Object.values(allSchools)) {
  allByName[name] = defensive_rebound_pct;
}
...
for (const ourSlug of OUR_TEAMS) {
  ...
  if (allSchools[ourSlug])
    val = allSchools[ourSlug].defensive_rebound_pct;
  ...
  if (val !== null) out[dataId] = { defensive_rebound_pct: val };
...
const summary = `Done! ${matched} of ${OUR_TEAMS.length} teams matched.`
  + (unmatched.length ? `\n\nNo match found for:\n  ${unmatched.join(', ')}` : '')
  + `\n\nSave output as: import/stats_adv_opp.json`;
